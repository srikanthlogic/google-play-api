...
export const retryStats = { attempted: 0, recovered: 0, exhausted: 0 };
...
export const retryCall = async (fn, deps = {}) => {
  ...
  for (let attempt = 0; attempt < retryConfig.maxAttempts; attempt++) {
    try {
      if (attempt > 0) retryStats.recovered++;
      return await fn();
    } catch (err) {
      lastErr = err;
  ...
  // Every attempt failed — the request is about to surface as a 502/504.
  retryStats.exhausted++;
  recordIntegrity({ kind: 'retry_exhausted', endpoint: 'upstream', detail: `all ${retryConfig.maxAttempts} attempts failed: ${lastErr && lastErr.message}` });
  throw lastErr;
};
