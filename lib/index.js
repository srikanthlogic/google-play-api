...
// C8: every scraper call runs under the upstream timeout budget so a slow
// upstream cannot exhaust workers. Timeouts reject with UpstreamTimeoutError
// -> 504 + Retry-After via the error middleware.
const gplay = new Proxy(gplayScraper, {
  get (target, prop) {
    const value = target[prop];
    if (typeof value === 'function') {
      return (...args) => cachedCall(prop, args, () => withTimeout(value.apply(target, args)));
    }
    return value;
  }
});
