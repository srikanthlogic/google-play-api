...
import { query, param, body, validationResult } from 'express-validator';import {
  MAX_PAGE_SIZE,
  DEFAULT_START,
  MAX_LIST_RESULTS,
  MAX_BATCH_APPS,
...
import { buildExportStream, EXPORT_MAX_REVIEWS } from './exportStream.js';
import { iteratePage } from './iterators.js';
import { buildHealthReport } from './health.js';
import { snapshotApp, getHistory, getChanges } from './history.js';
import { HistoryListSchema, ChangesReportSchema } from './schemas.js';
import { processReviews } from './reviewUtils.js';
import gplay from './gplayClient.js';

const router = Express.Router();

// H4 (#192): HTTP query values are always strings, while the scraper's zod
// schemas declare non-string fields (paginate/fullDetail: boolean, throttle:
// number). Spreading raw req.query into gplay.* calls therefore 500s on any
// declared-but-wrong-typed key once retries are exhausted. Handlers build
// scraper opts from explicit whitelists via whitelistedQuery(); on /v2,
// rejectUnknownQueryParams() answers 400 (RFC 9457 problem doc) for
// undeclared params, while the deprecated /api/ surface keeps its lenient
// historical posture (unknown params ignored) until sunset.
const rejectUnknownQueryParams = (req, res, allowed) => {
  const unknown = Object.keys(req.query || {}).filter(key => !allowed.includes(key));
  if (unknown.length === 0) return true;
  if (req.baseUrl === '/v2') {
    const error = new Error(`Unknown query parameter(s): ${unknown.sort().join(', ')} (supported: ${allowed.join(', ')})`);
...
const whitelistedQuery = (req, keys) => {
  const opts = {};
  for (const key of keys) {
    if (req.query[key] !== undefined && req.query[key] !== '') opts[key] = req.query[key];
  }
  return opts;
};
...
router.get('/apps/',
  ...
  function (req, res, next) {
    ...
    if (!rejectUnknownQueryParams(req, res, ['q', 'num', 'country', 'lang'])) return;
...
router.get('/apps/', function (req, res, next) {
  ...
  if (!rejectUnknownQueryParams(req, res, ['num', 'start', 'collection', 'category', 'age', 'country', 'lang'])) return;
  const listOpts = Object.assign(whitelistedQuery(req, ['collection', 'category', 'age', 'country', 'lang']), {
    num: Math.min(start + num, MAX_LIST_RESULTS)
  });
...
/* B3: streaming reviews export (NDJSON or CSV) */
router.get('/apps/:appId/reviews/export',
  param('appId').isString().trim().isLength({ min: 1 }).withMessage('appId is required'),
  query('format').optional().isIn(['ndjson', 'csv']).withMessage('format must be ndjson or csv'),
  query('sort').optional().isIn(['helpful', 'rated', 'newest']).withMessage('sort must be helpful, rated, or newest'),
  query('country').optional().matches(COUNTRY_CODE_RE).withMessage('country must be an ISO 3166-1 alpha-2 code'),
  query('lang').optional().matches(LANG_CODE_RE).withMessage('lang must be a BCP-47 tag'),
  handleValidationErrors,
  function (req, res, next) {
    const format = req.query.format === 'csv' ? 'csv' : 'ndjson';
    if (!rejectUnknownQueryParams(req, res, ['format', 'sort', 'country', 'lang'])) return;
    const iter = gplayScraper.reviewsIterator(Object.assign(whitelistedQuery(req, ['country', 'lang']), {
    ...
    res.setHeader('X-Export-Cap', String(EXPORT_MAX_REVIEWS));
    buildExportStream(iter, format)
      .on('error', next)
      .pipe(res);
  });

/* C5: upstream health snapshot — instant by default, ?probe=true does a live upstream fetch */
router.get('/health',
  query('probe').optional().isBoolean().withMessage('probe must be a boolean'),
  handleValidationErrors,
  function (req, res, next) {
    buildHealthReport({ probe: req.query.probe === 'true' })
      .then(report => res.json(report))
      .catch(next);
  });

/* B4: cursor-paginated search iterator */
router.get('/apps/search',
  query('q').isString().trim().isLength({ min: 1 }).withMessage('Search query must be a non-empty string'),
  query('pageSize').optional().isInt({ min: 1, max: 100 }).withMessage('pageSize must be between 1 and 100'),
  query('cursor').optional().isString().trim().matches(/^[A-Za-z0-9_-]+$/).withMessage('cursor must be a nextToken'),
  handleValidationErrors,
  function (req, res, next) {
    const params = {
      term: req.query.q,
      ...
      lang: req.query.lang,
      fullDetail: req.query.fullDetail === 'true',
      pageSize: req.query.pageSize ? parseInt(req.query.pageSize, 10) : undefined
    };
    iteratePage(gplayScraper, 'search', params, req.query.cursor || null)
      .then(({ results, nextToken }) =>
        res.json({ results: results.map(cleanUrls(req)), nextToken }))
      .catch(next);
  });

/* B4: cursor-paginated developer apps iterator */
router.get('/developers/:devId/apps',
  ...
  function (req, res, next) {
    ...
    iteratePage(gplayScraper, 'developer', params, req.query.cursor || null)
      .then(({ results, nextToken }) =>
        res.json({ results: results.map(cleanUrls(req)), nextToken }))
      .catch(next);
  });
...
router.get('/apps/:appId',
  ...
  function (req, res, next) {
    ...
    if (!rejectUnknownQueryParams(req, res, ['fields', 'country', 'lang'])) return;
    const opts = Object.assign(whitelistedQuery(req, ['country', 'lang']), {
      appId: req.params.appId
    });
    gplay.app(opts)
...
/* B9: snapshot timeline for one app (oldest first) */
router.get('/apps/:appId/history',
  param('appId').isString().trim().isLength({ min: 1 }).withMessage('appId is required'),
  handleValidationErrors,
  function (req, res, next) {
    getHistory(req.params.appId)
      .then((snapshots) => res.json({ results: snapshots }))
      .catch(next);
  });

/* B9: field-level changes between snapshots after ?since= (ISO date) */
router.get('/apps/:appId/changes',
  param('appId').isString().trim().isLength({ min: 1 }).withMessage('appId is required'),
  query('since').optional().isISO8601().withMessage('since must be an ISO 8601 date'),
  handleValidationErrors,
  function (req, res, next) {
    getChanges(req.params.appId, req.query.since)
      .then((changes) => res.json({ results: changes }))
      .catch(next);
  });

/* B9: snapshot timeline */
router.get('/apps/:appId/history',
  param('appId').isString().trim().isLength({ min: 1 }).withMessage('appId is required'),
  handleValidationErrors,
  function (req, res, next) {
    ...
    getHistory(appId)
      .then((snapshots) => {
        const body = { appId, snapshots };
        if (req.baseUrl === '/v2') {
          const parsed = HistoryListSchema.safeParse(body);
          if (!parsed.success) return next(new Error('History response failed schema validation'));
...
/* B9: field-level changes since a date */
router.get('/apps/:appId/changes',
  param('appId').isString().trim().isLength({ min: 1 }).withMessage('appId is required'),
  query('since').optional().isISO8601().withMessage('since must be an ISO 8601 date'),
  handleValidationErrors,
  function (req, res, next) {
    const appId = req.params.appId;
    getChanges(appId, req.query.since || null)
      .then((changes) => {
        const body = { appId, since: req.query.since || null, changes };
        if (req.baseUrl === '/v2') {
          const parsed = ChangesReportSchema.safeParse(body);
          if (!parsed.success) return next(new Error('Changes response failed schema validation'));
          return res.json(parsed.data);
        }
        res.json(body);
      })
...
router.get('/apps/:appId/similar',
  ...
  handleValidationErrors,
  function (req, res, next) {
    if (!rejectUnknownQueryParams(req, res, ['country', 'lang'])) return;
    const opts = Object.assign(whitelistedQuery(req, ['country', 'lang']), {
      appId: req.params.appId
    });
    gplay.similar(opts)
...
router.get('/apps/:appId/datasafety',
  ...
  function (req, res, next) {
    if (!rejectUnknownQueryParams(req, res, ['country', 'lang'])) return;
    const opts = Object.assign(whitelistedQuery(req, ['country', 'lang']), { appId: req.params.appId });
...
router.get('/apps/:appId/reviews',
  ...
  handleValidationErrors,
  function (req, res, next) {
    if (!rejectUnknownQueryParams(req, res, ['num', 'sort', 'country', 'lang', 'userdata', 'replies', 'nextPaginationToken'])) return;
    const opts = Object.assign(whitelistedQuery(req, ['country', 'lang', 'nextPaginationToken']), {
      appId: req.params.appId,
...
router.get('/developers/:devId/',
  ...
  handleValidationErrors,
  function (req, res, next) {
    if (!rejectUnknownQueryParams(req, res, ['num', 'country', 'lang'])) return;
    const opts = Object.assign(whitelistedQuery(req, ['country', 'lang']), {
      devId: req.params.devId,
      num: req.query.num ? parseInt(req.query.num, 10) : undefined
    });
