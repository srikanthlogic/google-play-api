...
const trip = (now = Date.now()) => {
  ...
  state.phase = 'open';
  // C5: a trip is an upstream-integrity event — surfaced by /v2/health.
  recordIntegrity({ kind: 'breaker_open', endpoint: 'upstream', detail: `failure threshold reached (${breakerConfig.failureThreshold} in ${Math.round(breakerConfig.windowMs / 1000)}s)` });
  state.openedAt = now;
};
