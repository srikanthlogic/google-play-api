...
export class UpstreamTimeoutError extends AppError {
  constructor(message = 'The upstream service did not respond within the timeout budget') {
    super(message, 504);
  }
}
...
const ERROR_TYPES = {
  ...
  504: 'https://api.google-play-api.dev/problems/upstream-timeout',
...
export const problemDetails = (err, req, status = getErrorStatusCode(err)) => {
  ...
  if (status === 504) {
    response.retryAfter = parseInt(process.env.UPSTREAM_TIMEOUT_RETRY_AFTER || '5', 10);
  }
  return response;
};
